# =====================================================================
# GIP SESAN OBSERVATORY - VERSION PROFESSIONNELLE
# =====================================================================
library(shiny)
...
ui <- dashboardPage(
  ...
  dashboardHeader(
    ...
    tags$li(class = "dropdown",
            tags$style(".main-header .navbar-custom-menu>.navbar-nav>li>.dropdown-menu {width:300px;}"),
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("📋 Rapport exécutif", tabName = "report", icon = icon("file-pdf"))
    )
  ),
    ...
    tabItems(
      ...
      tabItem(tabName = "overview",
        ...
        fluidRow(
          ...
          box(
            title = "🎯 Indicateurs de performance", status = "info",
            solidHeader = TRUE, width = 4,
            gaugeOutput("performance_gauge")
          )
        ),
        
        fluidRow(
          box(
            title = "📡 Flux temps réel", status = "warning", 
      ...
      # Cartographie (version sans leaflet)
      tabItem(tabName = "map",
        fluidRow(
          box(
...
server <- function(input, output, session) {
  ...
  output$mes_trends <- renderPlotly({
    ...
    ggplotly(p) %>%
      layout(
        annotations = list(
          list(x = 6, y = max(health_data()$mes_adoptions) * 0.9,
               text = "📈 Accélération Q2", showarrow = FALSE)
        )
      )
  })
  ...
  # Geographic distribution (sans leaflet)
